...
1. **Given** no query has been sent yet, **When** the page loads, **Then** a centered empty state with brand mark and four selectable example questions is shown.
...
- **FR-011**: System MUST provide an initial empty state with a centered brand visual and four example questions that can populate the input when selected.
...
- **Chat Session**: A single active conversation context containing a conversation identifier and ordered messages.
...
- **Example Prompt**: A pre-defined starter question shown in the empty state and insertable into the input field.

## Assumptions

- The backend endpoint is reachable from the public frontend origin and is configured to allow cross-origin browser access.
- A single-browser-session conversation scope is sufficient for demo purposes.
