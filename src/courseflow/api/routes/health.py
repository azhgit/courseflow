...
async def health_check(
    ...
    try:
        ...
        health_status["services"]["chromadb"] = {"status": "ok", "document_count": collection_count}
    except Exception as e:
        health_status["status"] = "degraded"
        health_status["services"]["chromadb"] = {"status": "error", "error": str(e)}

    # Check SQLite
    try:
        # Simple query count check
        query_count = await query_repo.get_recent_query_count(hours=24)
        health_status["services"]["sqlite"] = {"status": "ok", "queries_last_24h": query_count}
    except Exception as e:
        health_status["status"] = "degraded"
        health_status["services"]["sqlite"] = {"status": "error", "error": str(e)}
    ...
    try:
        ...
        requests_in_window = sum(1 for ts in rate_limiter.request_timestamps if ts >= cutoff)
        ...
        health_status["services"]["rate_limit"] = {
            ...
            "available_requests": max(0, rate_limiter.max_requests_per_minute - requests_in_window),
        }
    except Exception as e:
        health_status["status"] = "degraded"
